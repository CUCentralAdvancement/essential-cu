// CUSTOMIZED FOR CU for custom colors
// source: http://cdn.knightlab.com/libs/soundcite/latest/js/soundcite.js
// see CUSTOM.. notes - Voltage
...
(function(window, document, callback) { // http://stackoverflow.com/questions/2170439/how-to-embed-javascript-widget-that-depends-on-jquery-into-an-unknown-environmen
    ...
    // CUSTOM:
    var rgb = normalize_background_color('#d6cca6');
    if (rgb) {
    ...
    // CUSTOM:
    style.innerHTML = '.soundcite-loaded { background-color: rgba(' + SOUNDCITE_CONFIG.background_color + ',1) }';
    ...
    var update_playing_element = function(el, percentage) {
      ...
      // CUSTOM:
      // 'background: -webkit-linear-gradient(left, rgba( 207, 184, 124, 1)' + percentage + '%, rgba(' + color + ',.05)' + (percentage + 1) + '%);'
      'background: linear-gradient(to right, rgba( 207, 184, 124, 1)' + percentage + '%, rgba( 214, 204, 166, 1)' + (percentage + 1) + '%);'
    }
