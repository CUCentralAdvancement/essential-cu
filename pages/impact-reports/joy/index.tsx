...
export default function Index({cards}: HomepageProps) {
  return (
    <Layout>
          ...
          {Object.keys(cards).map((el, index) => {
            return (
              <Link key={index}
                ...
                <a>
                  <div className="rounded-3xl flex flex-col md:mx-3 my-3 md:my-0 shadow border h-full w-full">
                    <div style={{backgroundImage: `url('${cards[el].main_image.url}')`}}
                      ...
                      <span className="bg-gold text-center p-3 w-36 rounded-tr-full">{cards[el].campus}</span>
                        <span className="bg-white w-64 p-2 font-bold text-xl rounded-tr-full">{cards[el].title}</span>
                        {/*<span className="bg-white w-64 p-2 hidden md:inline-block">{cards[el].description.substr(0, 40)}</span>*/}
                    </div>
                  </div>
                </a>
              </Link>
      ...
      <Section type="2-col" sx="container space-x-4">
        <First sx="p-2">
          <TextBlock>
            <h3>This is really great</h3>
      ...
      <div className={"container mx-auto grid grid-cols-5 space-x-4"}>
          ...
          <Image src="https://place-hold.it/720x400/#ccc" alt="the alt text" sx={"col-span-3"}/>
      </div>
