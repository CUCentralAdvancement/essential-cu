...
export const getStoryByIdWithSlides = async (storySlug: string) => {
    ...
    const storyId = stories.items.find((entry: Entry<IStory> )=> (entry.fields.label.toLowerCase() === storySlug)).sys.id;
    const story = await client.getEntry<IStory>(storyId);
    return story;
}
...
export const getAllStoryIds = async () => {
    ...
    return stories.items.map( (entry: Entry<IStory> )=> ({params: { storyId: [entry.fields.label.toLowerCase()], id: entry.sys.id } }));
}